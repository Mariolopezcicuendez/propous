...
var interval_messages_no_readen = null;
...
$(document).ready(function() 
{
  ...
  if (!maintenance_page_showed()) 
  {
    ...
    get_statistics();
      ...
      if (exists_messages_no_readen())
      {
        print_messages_no_readen_in_messages_text();
      ...
      interval_messages_no_readen = setInterval( "check_if_i_have_no_readen_messages()"  , TIME_SECONDS_CHECK_USER_MESSAGES * 1000 );
  	}
  }
  else
  {
...
{
  if ((typeof interval_messages_no_readen !== "undefined") && (interval_messages_no_readen !== null) && (interval_messages_no_readen !== ''))
  {
    clearInterval(interval_messages_no_readen);
  }
...
function check_if_i_have_no_readen_messages()
{
  var user_id = $(".gdata_user_id").val();

  var req = {};
  req.url = '/message/get_count_no_readen_messages/'+user_id;
  req.type = 'GET';
  req.success = "success_check_if_i_have_no_readen_messages";
  req.error = "error_check_if_i_have_no_readen_messages";
  ajaxp(req);
}

function success_check_if_i_have_no_readen_messages(data)
{
  var messages = data.result;

  localStorage.setItem("messages_no_readen",messages);

  if (messages > 0)
  {
    if ($(".messages_main_link_page_text_messages_noreaden").text() !== '')
    {
      if (parseInt($(".messages_main_link_page_text_messages_noreaden").text()) < messages)
      {
    ...
    $(".messages_main_link_page_text_messages_noreaden").text(messages);
...
function error_check_if_i_have_no_readen_messages(data)
{
  
}

function exists_messages_no_readen()
{
  var messages_no_readen = localStorage.getItem("messages_no_readen");
  return (messages_no_readen > 0);
}

function print_messages_no_readen_in_messages_text()
{
  var messages_no_readen = localStorage.getItem("messages_no_readen");

  $(".messages_main_link_page_text_messages_noreaden").text(messages_no_readen);
  $(".messages_main_link_page_text_messages_noreaden").removeClass("hidden");
}
...
function play_message_received()
{
