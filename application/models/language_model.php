...
class Language_model extends CI_Model 
{
  ...
  {
    $lang = getActLang();
    $query = $this->db->query("SELECT `language`.id, tag, name FROM `language` JOIN language_i18n ON ( language.id = language_i18n.language_id ) WHERE lang = '{$lang}' ORDER BY name");
    if ($query->num_rows() > 0)
    {
      $results = array();
  ...
  function get_spoken($user_id)
  {
    $lang = getActLang();
    $query = $this->db->query("
      SELECT user_language.id, user_language.user_id, user_language.language_id, name, quality
      FROM  `user_language` 
      JOIN language_i18n ON ( user_language.language_id = language_i18n.language_id ) 
  ...
  function save_spoken($user_id)
  {
  	$language = $this->input->post('user_spoken_language');
		list($id_language,$level_language) = explode(":",$language);
  	
  	$user_language = array();

    $this->db->trans_begin();

		$query = $this->db->query("SELECT id FROM `user_language` WHERE user_id = {$user_id} AND language_id = {$id_language}");
    if ($query->num_rows() > 0)
    {
    	// UPDATE
    	$row = $query->row();
    	$id_row = $row->id;
			...
			$user_language['quality'] = $level_language;

    	$this->db->where('id', $id_row);
    	$this->db->update('user_language', $user_language);
    }
    else
    {
    	// INSERT
			$user_language['user_id'] = $user_id;
			$user_language['language_id'] = $id_language;
			$user_language['quality'] = $level_language;

			$this->db->insert('user_language', $user_language);
    }
    ...
    if ($this->db->trans_status() === FALSE)
    {
      ...
      throw new Exception(lang('exception_error_1302'), 1302);
    }
    else
    {
  ...
  function delete_spoken($spoken_id)
  {
  	$this->db->where('id', $spoken_id);
    $result = $this->db->delete('user_language');
    if ($result)
    {
      return $result;
    }

    throw new Exception(lang('exception_error_1301'), 1301);
  }
