...
class State_model extends CI_Model 
{
  ...
  function all_from_country($country_id)
  {
  	$query = $this->db->query("SELECT id, name, country_id FROM `state` WHERE country_id = {$country_id} ORDER BY name");
    if ($query->num_rows() > 0)
    {
      $results = array();
  ...
  {
    $query = $this->db->query("SELECT name FROM `state` WHERE id = {$state_id}");
    if ($query->num_rows() > 0)
    {
      $row = $query->row();
      return $row->name;
